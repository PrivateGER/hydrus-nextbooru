...
/**
 * Error thrown when an admin-supplied base URL is not a usable endpoint.
 */
...
/**
 * Validate that an admin-supplied base URL is a well-formed absolute http(s)
 * URL. This is a format check only — the app is self-hosted and the admin is
 * trusted to point it at any network location (LAN, tailnet, loopback, ...),
 * so no SSRF/private-range filtering is applied.
 *
 * @throws InvalidBaseUrlError if the URL is malformed or not http(s).
 * @returns the trimmed URL string when valid.
 */
export function validateBaseUrlFormat(rawUrl: string): string {
  ...
  try {
    ...
    throw new InvalidBaseUrlError("Base URL is not a valid absolute URL.");
  }
  ...
  // fetch() throws a TypeError when constructing a request from a URL with
  // embedded credentials, so such a URL can never work at call time. Use an
  // API key field for authentication instead.
  if (url.username || url.password) {
    throw new InvalidBaseUrlError(
      "Base URL must not contain embedded credentials; use the API key field."
    );
  }
